...
router.route('/remove/:keyperKey/:app')
	.post(function(req, res, next) {
		...
		mongoose.model('Secret').find({keyper_key: keyperKey, app: app}).remove().exec(function(err, data){
			if (err || !data) {
				winston.error('Failed to remove app=' + app + ' for keyperKey=' + keyperKey);
				winston.error(err);
				res.json('Failed to remove application=' + app + ' does this application exist?');
			} else {
				winston.info('Successfully removed key, if it existed');
				res.json('Removed application=' + app + ' if it existed.');
			}
		});
	})
...
router.route('/:keyperKey')
    ...
    .get(function(req, res, next) {
			...
			mongoose.model('Secret').find({keyper_key: keyperKey}, function(err, secrets){
				if (err) {
					winston.error(err);
					res.json('Failed to get data for your keyper key. Does this key exist?');
				}
